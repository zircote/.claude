...
description: Strategic project planning with Socratic requirements elicitation. Produces PRD, technical architecture, and implementation plan with full artifact lifecycle management. Part of the /arch suite - use /arch/c to complete projects, /arch/s for status.
model: claude-opus-4-5-20251101
allowed-tools: Read, Write, Edit, Bash, Glob, Grep, Task, WebSearch, WebFetch, TodoRead, TodoWrite
---

# /arch/p - Strategic Project Planner
...
- **Format**: `ARCH-[YYYY]-[MM]-[DD]-[SEQ]`
- **Example**: `ARCH-2025-12-11-001`
...
**This command defers to the `worktree-manager` skill for all worktree operations.**

If the skill is available at `~/.claude/skills/worktree-manager/SKILL.md`:
1. Read the skill documentation first
2. Use its established workflow and commands
3. Follow its naming conventions and branch patterns

If the skill is not available, provide basic guidance and recommend installing it.

### Critical: New Session Required After Worktree Creation

**⚠️ IMPORTANT: If a worktree is created, the user MUST continue in the NEW terminal.**
...
🖥️  A new terminal window has been opened with Claude Code already running 
...
**After executing launch-agent.sh and displaying this message, STOP. Do not proceed with planning.**
...
```bash
# Check if worktree-manager skill exists
WORKTREE_SKILL="~/.claude/skills/worktree-manager/SKILL.md"
if [ -f "$WORKTREE_SKILL" ]; then
  echo "SKILL_AVAILABLE=true"
else
  echo "SKILL_AVAILABLE=false"
fi
...
```
IF worktree-manager skill is available:
...
IF skill NOT available AND on protected branch (main/master/develop):
  → RECOMMEND installing worktree-manager skill
...
| `arch/[slug]` | Alternative planning prefix |
...
1. **Generate Project Identifiers**:
   ...
   Project ID: ARCH-[Date]-[Seq]
   Slug: [Derive from project seed, lowercase, hyphens, max 30 chars]
   ```

2. **Create Directory Structure**:
   ```bash
   mkdir -p docs/architecture/active/[YYYY-MM-DD]-[slug]
   ```
...
```bash
find docs/architecture -name "*[slug]*" -type d 2>/dev/null
grep -r "[project keywords]" docs/architecture/*/README.md 2>/dev/null
```
...
5. **Update CLAUDE.md** (if exists):
   ```markdown
   ## Active Architecture Projects
   - `docs/architecture/active/${DATE}-${SLUG}/` - ${PROJECT_NAME} (in-review)
...
**Defer to `worktree-manager` skill if available.**

```bash
# Check for worktree-manager skill
WORKTREE_SKILL="~/.claude/skills/worktree-manager/SKILL.md"
if [ -f "$WORKTREE_SKILL" ]; then
  echo "worktree-manager skill available - reading documentation"
  # READ THE SKILL and follow its workflow
fi
...
```
IF worktree-manager skill exists:
  → READ ~/.claude/skills/worktree-manager/SKILL.md
  → FOLLOW its workflow and conventions
  → IF worktree is CREATED:
    → Display session restart instructions
    → STOP COMPLETELY - do not proceed
    → User must exit, cd to worktree, start new claude session, re-run /arch/p
...
IF skill NOT available:
  ...
  IF BRANCH starts with [plan/, arch/, feature/]:
    → PROCEED with "✓ Appropriate branch: ${BRANCH}"
```

**⚠️ CRITICAL: If a worktree is created during this step, you MUST stop and instruct the user to restart. Do NOT continue with planning in the current session.**
...
mkdir -p "docs/architecture/active/${DATE}-${SLUG}"
...
If a worktree is created (either via skill or fallback):

1. **Execute the launch-agent.sh script** from worktree-manager skill
...
~/.claude/skills/worktree-manager/scripts/launch-agent.sh \
  "[worktree-path]" \
  "Architecture planning: [project-name]"
```
...
│  Switch to the new terminal window and run:                        │
│                                                                     │
│     /arch/p [your original project idea]                           │
│                                                                     │
...
`docs/architecture/active/2025-12-11-[slug]/` with Project ID `ARCH-2025-12-11-001`.
...
```
⚠️ Continuing on `main` as requested. Note that planning artifacts will be 
